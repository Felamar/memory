...
public class MemoryModel extends Thread {
  ...
  public static byte address_radix;
  private MemoryView control_panel;
  private String output_file;
  private String commands_path;
  private String config_path;
  private Vector<Page> pages_vector;
  private Vector<Command> instructions_vector;
  private static String lineSeparator = System.getProperty("line.separator"); // why is "final"?

  MemoryModel(String commands_path, String config_path){
    //this.init(commands_path, config_path);
    this.runs = 0;
    this.run_cycles = 0;
    ...
    this.address_radix = 10;
    this.output_file = "tracefile";
    this.commands_path = commands_path;
    this.config_path = config_path;
    this.pages_vector = new Vector<Page>();
    this.instructions_vector = new Vector<Command>();
    this.lineSeparator = System.getProperty("line.separator");

    this.readCommandsFile(commands_path);

    this.readConfFile(config_path);
  ...
  public void readCommandsFile(String commands_path){
    // Read commands file
    try {
      ...
      while (commands_scanner.hasNextLine()) {
        ...
        if (line[type].toLowerCase().equals("random")) {
          instruction = new Command(line[command], Common.randomLong(address_limit));
        } else if (line.length == 3) {
          instruction = new Command(line[command], line[type], line[address_1], null);
          instruction.isInRange(address_limit);
        }else{
          instruction = new Command(line[command], line[type], line[address_1], line[address_2]);
          instruction.isInRange(address_limit);
        }
  ...
  public void readConfFile(String config_path){
      ...
      try {
        ...
        while (config_scanner.hasNextLine()) {
          ...
          switch (line[0].toLowerCase()) {
            ...
            case "addressradix":
              MemoryModel.address_radix = Byte.parseByte(line[1]);
              break;
  ...
  public void step() {
    Command instruction = this.instructions_vector.elementAt(this.runs);
  ...
  public void reset() {
    ...
    //this(commands_path, config_path); // reload files: make a method to do that
    this.readCommandsFile(this.commands_path);
    this.readConfFile(this.config_path);
  }
  ...
  public int getNoVirtualPages(){
    return this.no_virtual_pages;
  }
  ...
  public static long str2int(String s){
    int i = 0;
